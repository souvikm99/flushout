...
function validEmailAddress(value) {
  ...
  if (!domain.includes(".") || domain.length > 253 || local.startsWith(".") || local.endsWith(".") || local.includes("..")) return false;
...
async function sendCompletionEmail(env, recipient, details) {
  ...
  if (port !== 465 || !validEmailAddress(fromEmail) || !validEmailAddress(recipient)) {
    throw Object.assign(new Error("notification_unavailable"), { status: 503 });
  }
...
export const internals = { normalizeUsername, normalizeSessionName, validEmailAddress, b64url, fromB64url, securityHeaders, signTicket, verifyTicket, smtpConversation };
